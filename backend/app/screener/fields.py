...
- Pass1 (全市场快照过滤): change_pct, amount, amplitude - fuyao prices/snapshot 全市场模式
  amplitude 由 (high-low)/prev_close*100 计算得出
- Pass2 (幸存者逐个查): roe (financials), macd_golden_cross/rsi_oversold/boll_breakout (indicators)
- 不可用: pe_ttm/pb/total_market_cap (估值接口未上线), turnover (snapshot无此字段), industry (无行业接口)
"""
...
FIELDS: list[FilterField] = [
    ...
    FilterField(name="amplitude", label="振幅", type="number_range", category="行情表现",
                unit="%", constraints={"min": 0, "max": 20, "step": 0.5},
                available=True),  # computed from (high-low)/prev_close*100
    # 基本面 - Pass2 逐个查
    FilterField(name="roe", label="ROE", type="number_range", category="基本面",
                unit="%", constraints={"min": 0, "max": 100, "step": 1}),
    # 技术信号 - Pass2 计算
...
SORTABLE = ["change_pct", "roe", "amplitude", "amount"]
