...
export default function NewsPanel() {
  ...
  const fetchNews = async (refresh = false) => {
    ...
    try {
      ...
      if (d.news && d.news.length > 0) { setNews(d.news); setError('') }
      else { setError(d.error || '暂无相关新闻') }
    } catch { setError('获取新闻失败') }
    setLoading(false)
  }

  const toggleExpand = (idx: number) => { setExpanded(expanded === idx ? null : idx); setAnalyzing(null) }

  const analyze = async (idx: number) => {
    const item = news[idx]; if (!item || !stock) return
    ...
    try {
      ...
      while (true) {
        ...
        for (const l of lines) {
          if (l.startsWith('data: ')) try { const j = JSON.parse(l.slice(6)); if (j.done) return; if (j.content) setAnalysis(p => p + j.content) } catch {}
        }
      }
  ...
  const fmtDate = (pubAt: string) => {
    ...
    if (d === new Date(Date.now() - 86400000).toISOString().slice(0, 10)) return '昨天 ' + pubAt.slice(11, 16)
    return d
  }
      ...
      <div className="news-header">
        ...
        <div className="controls">
          <select value={days} onChange={e => { setDays(Number(e.target.value)); fetchNews(true) }}>
            <option value={1}>近1天</option><option value={3}>近3天</option>
            <option value={7}>近7天</option><option value={14}>近14天</option><option value={30}>近30天</option>
          </select>
          <button onClick={() => fetchNews(true)} disabled={loading}>{loading ? '搜索中...' : '刷新'}</button>
        </div>
      </div>
            ...
            {expanded === i && (
              <div className="news-detail">
                ...
                <div className="actions">
                  {item.url && <a href={item.url} target="_blank" rel="noopener" onClick={e => e.stopPropagation()}>🔗 阅读原文</a>}
                  <button onClick={e => { e.stopPropagation(); analyze(i) }}>🤖 AI 分析</button>
                </div>
                {analyzing === i && <div className="news-analysis">{analysis || '分析中...'}</div>}
              </div>
            )}
          </div>
        ))}
        {!loading && news.length === 0 && !error && <div className="news-empty">暂无新闻</div>}
      </div>
    </div>
  )
}
